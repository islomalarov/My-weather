...
export function getWeak(length, dt) {
    ...
    if (currentDate === changeDate && (currentHours >= 22 || currentHours <= 6)) {
        ...
        weak = new Intl.DateTimeFormat('en-US', { weekday: length }).format(new Date(dt * 1000)).toUpperCase();
    }
...
export function getMonth(length, dt) {
    let month = new Intl.DateTimeFormat('en-US', { month: length, day: 'numeric' }).format(new Date(dt * 1000));
...
export function timeConvert(dt, minute) {
    if (minute) {
        return new Intl.DateTimeFormat('en-AU', { hour: 'numeric', minute: minute }).format(new Date(dt * 1000));
    }
